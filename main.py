...
# --- MODIFICATION: Added Last 4 Digits ---
ALL_COLUMNS = [
...
# (No changes here, just adding comments)
...
def load_data():
    ...
    Handles migrations by adding any missing columns.
    """
    try:
    ...
    if "Bonus Status" not in df.columns:
        df["Bonus Status"] = ""
    # --- NEW: Add Last 4 Digits column if missing ---
...
def get_card_mapping():
    """Scans the IMAGE_DIR and creates a mapping for the 'Choose from list' dropdown."""
    card_mapping = {}
    try:
...
def show_add_card_form(card_mapping):
    ...
    if submitted:
        ...
        # --- NEW: Validate Last 4 Digits (must be 4 digits or empty) ---
        ...
        new_card = {
            ...
            "Last 4 Digits": last_4_digits  # <-- NEW
        }
...
def show_edit_form():
    ...
    with st.form("edit_card_form"):
        ...
        last_4_digits = st.text_input(
            "Last 4 Digits (Optional)", 
            value=card_data.get("Last 4 Digits", ""),  # <-- Load existing
            max_chars=4, 
            help="Useful for tracking supplementary cards."
        )
        # --- End of new ---
    ...
    if submitted:
        ...
        all_cards_df.loc[card_index, "Last 4 Digits"] = last_4_digits  # <-- NEW
...
def show_dashboard(all_cards_df):
    ...
    for index, card_row in cards_to_show_df_sorted.iterrows():
        ...
        with col2: # Info
            ...
            # --- MODIFICATION: New Title/Expiry UI ---
            ...
            # Use a single markdown block with CSS for layout
            st.markdown(
                f"""
            ...
            with b_col3: # Cancel/Re-activate
                if is_cancelled:
            ...
            with st.expander("Show All Dates and Details"):
                # Expiry is removed from here since it's now at the top
...
def show_details_page():
    ...
    # --- MODIFICATION: Updated date formatter to return "" instead of "N/A" ---
    strftime_code = STRFTIME_MAP[st.session_state.date_format]
    def format_date_display(date_val):
        """Formats a date for display, returning an empty string if null."""
        if pd.isna(date_val):
            return ""  # Return blank instead of "N/A"
        return pd.to_datetime(date_val).strftime(strftime_code)
    # --- End of modification ---
    ...
    with col2:
        # --- MODIFICATION: Build title with Last 4 Digits ---
    ...
    if not bonus_offer and min_spend == 0.0:
        ...
        # This will now show BLANK instead of "N/A"
    ...
    with st.form("delete_tags_form"):
        ...
        if submitted_delete and tags_to_delete:
            ...
            if save_tags(tags_to_keep):
                ...
                # Remove deleted tags from all cards
                df = load_data()
